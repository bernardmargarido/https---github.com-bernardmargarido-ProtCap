 ...
 *
 * 1 - Add the files JSON.prw and SHash.prw to your project.
 *
 * 2 - Compile JSON.prw and SHash.prw.
 *
 ...
 *
 *
 * Note: Thanks to the preprocessor we can call user_functions without
...
Function ToJson(uAnyVar, lEnvNull)
...
#ENDIF
	...
	if cType == "C"
		cRet := '"' + rtrim(EscpJsonStr(uAnyVar)) + '"'
	elseif cType == "N"
	...
	elseif cType == "A"
		...
		For nI := 1 to len(uAnyVar)
			cValue := ToJson(uAnyVar[nI], lEnvNull)
	...
	elseif cType =="O"
		if uAnyVar:cClassName == "SHASH"
			...
			For nI := 1 to len(uAnyVar:aData)
				cValue := ToJson(uAnyVar:aData[nI][SPROPERTY_VALUE], lEnvNull)
				if cValue <> "null" .or. lEnvNull
					if(lFirst)
						lFirst := .F.
					else
					...
					cRet += '"'+ EscpJsonStr(uAnyVar:aData[nI][SPROPERTY_KEY]) +'":'+ cValue
				endif
			Next
			cRet += '}'
	   	else
...
#IFDEF __HARBOUR__
	elseif cType =="H"  // Harbour Associative Array.
		...
		For nI := 1 to len(aAsKeys)
			cValue := ToJson(uAnyVar[aAsKeys[nI]], lEnvNull)
			if cValue <> "null" .or. lEnvNull
				if(lFirst)
					lFirst := .F.
				else
				...
				cRet += '"'+ EscpJsonStr(aAsKeys[nI]) +'":'+ cValue
			endif
		Next
		cRet += '}'
#ENDIF
